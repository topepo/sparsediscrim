...
#' dlda_out <- lda_diag(species ~ ., data = penguins[-predict_rows, ])
#' predicted <- predict(dlda_out, penguins[predict_rows, -1], type = "class")
#'
#' dlda_out2 <- lda_diag(x = penguins[-predict_rows, -1], y = penguins$species[-predict_rows])
#' predicted2 <- predict(dlda_out2, penguins[predict_rows, -1], type = "class")
