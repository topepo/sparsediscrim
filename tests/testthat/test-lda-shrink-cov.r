...
test_that("The SDLDA classifier works properly on the iris data set", {
  ...
  predicted <- predict(sdlda_out, iris[-train, -5], type = "prob")

  sdlda_out2 <- lda_shrink_cov(x = iris[train, -5], y = iris[train, 5])
  predicted2 <- predict(sdlda_out2, iris[-train, -5], type = "prob")
...
test_that("The SDLDA classifier works properly when 1 feature used", {
  ...
  predicted_cls   <- predict(sdlda_out, iris[-train, 1, drop = FALSE])
  predicted_prob  <- predict(sdlda_out, iris[-train, 1, drop = FALSE], type = "prob")
  predicted_score <- predict(sdlda_out, iris[-train, 1, drop = FALSE], type = "score")
